...
===============
*/
static int UI_IsPlayerAdmin( void )
{
	uiClientState_t	cs;
	...
	admin =  atoi(Info_ValueForKey(info, "adminref"));
	if ( admin != 0 )
	{
		return admin;
	}
	clan =  atoi(Info_ValueForKey(info, "clan"));
	if ( clan )
	{
		return 9;		// Hack value
	}
...
/*
===============
UI_BuildAdminOptions
===============
*/
static void UI_BuildAdminOptions( void ) //01-22-05 moved around by pimpy
{
	strcpy( uiInfo.adminActionNames[0], "Kick" );
	strcpy( uiInfo.adminActionNames[1], "Ban" );
	strcpy( uiInfo.adminActionNames[2], "Force to Red" );
	strcpy( uiInfo.adminActionNames[3], "Force to Blue" );
	strcpy( uiInfo.adminActionNames[4], "Force to Spec" );
	strcpy( uiInfo.adminActionNames[5], "Invite to Red" );
	strcpy( uiInfo.adminActionNames[6], "Invite to Blue" );
	strcpy( uiInfo.adminActionNames[7], "Switch" );

	strcpy( uiInfo.adminActionNames[8], "Penalty" );
	strcpy( uiInfo.adminActionNames[9], "UnPenalty" );
	strcpy( uiInfo.adminActionNames[10], "Mute" );
	strcpy( uiInfo.adminActionNames[11], "NoVote" );
	strcpy( uiInfo.adminActionNames[12], "Bait" );
	strcpy( uiInfo.adminActionNames[13], "Dummy" );
	strcpy( uiInfo.adminActionNames[14], "Launch" );
	strcpy( uiInfo.adminActionNames[15], "Strike" );
	strcpy( uiInfo.adminActionNames[16], "Surrender" );
	strcpy( uiInfo.adminActionNames[17], "Fry" );
	strcpy( uiInfo.adminActionNames[18], "Explode" );
	strcpy( uiInfo.adminActionNames[19], "Plant" );
	strcpy( uiInfo.adminActionNames[20], "Respawn" );
	strcpy( uiInfo.adminActionNames[21], "Telefrag" );
	strcpy( uiInfo.adminActionNames[22], "Slap: Phone" );
	strcpy( uiInfo.adminActionNames[23], "Slap: Saucepan" );
	strcpy( uiInfo.adminActionNames[24], "Slap: Club" );
	strcpy( uiInfo.adminActionNames[25], "Slap: Fish" );
	strcpy( uiInfo.adminActionNames[26], "Slap: Ping" );
	strcpy( uiInfo.adminActionNames[27], "Backfire" );


	strcpy( uiInfo.adminMessageNames[0], "All players" );
	strcpy( uiInfo.adminMessageNames[1], "Peers only" );
}
...
static void UI_RunMenuScript(const char **args)
{
	...
	adminval = UI_IsPlayerAdmin();
	if ( adminval == -1 || adminval == 2 || adminval == 3 )
	{
		prefix = "adm";
	}
	else if ( adminval == 9 )
	...
	if (String_Parse(args, &name))
	{
		...
		else if (Q_stricmp(name, "rocmodSetup") == 0)
		{
			adminval = UI_IsPlayerAdmin();
			trap_Cvar_Set( "ui_playerIsAdmin", va("%i", adminval) );
		}
		else if (Q_stricmp(name, "adminCancelVote") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s cancelvote\n", prefix) );
		}
		else if (Q_stricmp(name, "adminPassVote") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s passvote\n", prefix) );
		}
		else if (Q_stricmp(name, "adminSuspend") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s suspend\n", prefix) );
		}
		else if (Q_stricmp(name, "adminSendMessage") == 0)
		{
			char text[MAX_SAY_TEXT];

			trap_Cvar_VariableStringBuffer( "ui_AdminMessage", text, sizeof(text) );
			if ( uiInfo.adminMessageIndex == 0 )
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("%s talk %s\n", prefix, text) );
			}
			else
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("%s chat %s\n", prefix, text) );
			}
			trap_Cvar_Set( "ui_AdminMessage", "" );
		}
		else if (Q_stricmp(name, "adminDoAction") == 0)
		{
			if ( uiInfo.adminActionIndex >=0 && uiInfo.adminActionIndex < 28 && uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount )
			{
				char clientnum[3];
				...
				switch ( uiInfo.adminActionIndex )
				{
					case 0:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s kick %s\n", prefix, clientnum) );	// 01-22-05 moved around by pimpy
						break;
					case 1:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s ban %s 0\n", prefix, clientnum) );
						break;
					case 2:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s forceteam %s r\n", prefix, clientnum) );
						break;
					case 3:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s forceteam %s b\n", prefix, clientnum) );
						break;
					case 4:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s forceteam %s s\n", prefix, clientnum) );
						break;
					case 5:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s invite %s red\n", prefix, clientnum) );
						break;
					case 6:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s invite %s blue\n", prefix, clientnum) );
						break;
					case 7:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s switch %s\n", prefix, clientnum) );
						break;
					case 8:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s penalty %s\n", prefix, clientnum) );
						break;
					case 9:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s unpenalty %s\n", prefix, clientnum) );
						break;
					case 10:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s mute %s\n" , prefix, clientnum) );
						break;
					case 11:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s novote %s 10\n", prefix, clientnum) );
						break;
					case 12:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s bait %s\n", prefix, clientnum) );
						break;
					case 13:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s dummy %s\n", prefix, clientnum) );
						break;
					case 14:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s launch %s\n", prefix, clientnum) );
						break;
					case 15:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s strike %s\n", prefix, clientnum) );
						break;
					case 16:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s surrender %s\n", prefix, clientnum) );
						break;
					case 17:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s fry %s\n", prefix, clientnum) );
						break;
					case 18:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s explode %s\n", prefix, clientnum) );
						break;
					case 19:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s plant %s\n", prefix, clientnum) );
						break;
					case 20:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s respawn %s\n", prefix, clientnum) );
						break;
					case 21:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s telefrag %s\n", prefix, clientnum) );
						break;
					case 22:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s slap %s 1\n", prefix, clientnum) );
						break;
					case 23:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s slap %s 2\n", prefix, clientnum) );
						break;
					case 24:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s slap %s 3\n", prefix, clientnum) );
						break;
					case 25:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s slap %s 4\n", prefix, clientnum) );
						break;
					case 26:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s slap %s 5\n", prefix, clientnum) );
						break;
					case 27:
						trap_Cmd_ExecuteText( EXEC_APPEND, va("%s backfire %s\n", prefix, clientnum) );
						break;
				}
			}
		}
		else if (Q_stricmp(name, "adminSetMap") == 0)
		{
			if (ui_currentNetMap.integer >=0 && ui_currentNetMap.integer < uiInfo.mapCount)
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("%s map %s\n", prefix, uiInfo.mapList[ui_currentNetMap.integer].mapLoadName) );
			}
		}
		else if (Q_stricmp(name, "adminRestartMap") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s restart\n", prefix) );
		}
		else if (Q_stricmp(name, "adminNextMap") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s mapcycle\n", prefix) );
		}
		else if (Q_stricmp(name, "adminEndMap") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s endmap\n", prefix) );
		}
		else if (Q_stricmp(name, "adminSwapTeams") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s swapteams\n", prefix) );
		}
		else if (Q_stricmp(name, "adminShuffleTeams") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s shuffleteams\n", prefix) );
		}
		else if (Q_stricmp(name, "adminEvenTeams") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s eventeams\n", prefix) );
		}
		else if (Q_stricmp(name, "adminExtendTime") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s extendtime 15\n", prefix) );
		}
		else if (Q_stricmp(name, "adminDoWarmup") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s warmup 1\n", prefix) );
		}
		else if (Q_stricmp(name, "adminLockRed") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s lock red\n", prefix) );
		}
		else if (Q_stricmp(name, "adminLockBlue") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("%s lock blue\n", prefix) );
		}
		else if (Q_stricmp(name, "adminGametype") == 0)
		{
			if (ui_netGameType.integer >= 0 && ui_netGameType.integer < bg_gametypeCount )
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("%s gametype %s\n", prefix, bg_gametypeData[ui_netGameType.integer].name) );
			}
		}
		else if (Q_stricmp(name, "adminTimelimit") == 0)
		{
			int value = (int)trap_Cvar_VariableValue ( "ui_about_timelimit" );
			if (value >= 0 )
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("%s timelimit %d\n", prefix, value) );
			}
		}
		else if (Q_stricmp(name, "adminScorelimit") == 0)
		{
			int value = (int)trap_Cvar_VariableValue ( "ui_about_scorelimit" );
			if (value >= 0 )
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("%s scorelimit %d\n", prefix, value) );
			}
		}
...
static int UI_FeederCount(float feederID)
{
	...
	switch ( (int)feederID )
	{
		...
		case FEEDER_ADMIN_ACTIONS:
			UI_BuildAdminOptions();
			return 27;

		case FEEDER_ADMIN_MESSAGE:
			UI_BuildAdminOptions();
			return 2;
	}
	...
	*handle = -1;
	...
	switch ( (int)feederID )
	{
		...
		case FEEDER_ADMIN_ACTIONS:
			if (index >= 0 && index < 28)
			{
				return uiInfo.adminActionNames[index];
			}
			break;
		...
		case FEEDER_ADMIN_MESSAGE:
			if (index >= 0 && index < 2)
			{
				return uiInfo.adminMessageNames[index];
			}
			break;
...
static void UI_FeederSelection(float feederID, int index)
{
	...
	switch ( (int)feederID )
	{
		...
		case FEEDER_ADMIN_ACTIONS:
			uiInfo.adminActionIndex = index;
			break;

		case FEEDER_ADMIN_MESSAGE:
			uiInfo.adminMessageIndex = index;
			break;
...
static cvarTable_t cvarTable[] =
{
	...
	{ &ui_hudFiles, "cg_hudFiles", "ui/rochud.txt", CVAR_ARCHIVE},
	...
	{ &ui_playerIsAdmin,	"ui_playerIsAdmin",		"0", CVAR_ROM|CVAR_INTERNAL },
	{ &ui_AdminMessage,		"ui_AdminMessage",		"", CVAR_ROM|CVAR_INTERNAL },
