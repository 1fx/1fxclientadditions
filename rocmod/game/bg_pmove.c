...
static void PM_BeginWeaponChange(int weapon)
{
	...
	// #CL_ADD
	if( pm->ps->weapon >= WP_M84_GRENADE && pm->ps->weapon <= WP_MDN11_GRENADE && pm->ps->clip[ATTACK_NORMAL][pm->ps->weapon] <= 0 )
	// #END CL_ADD
	{
...
void PM_StartRefillClip ( attackType_t attack )
{
	...
	clip  = weaponData[pm->ps->weapon].attack[attack].clipSize;
	ammo = clip;
	clip -= pm->ps->clip[attack][pm->ps->weapon];

	if ( pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex] < clip )
	{
		clip = pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex];
	}

	pm->ps->clip[attack][pm->ps->weapon]+=clip;
	if ( g_realisticAmmo.integer )
	{
		if ( pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex] < ammo )
		{
			ammo = pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex];
		}
		pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex] -= ammo;
	}
	else
	{
		pm->ps->ammo[weaponData[pm->ps->weapon].attack[attack].ammoIndex] -= clip;
	}
...
int PM_GetAttackButtons(void)
{
	...
	switch( pm->ps->firemode[pm->ps->weapon] )
	{
		...
		case WP_FIREMODE_SINGLE:
			break;
#ifdef SMKWEAPONS					//
		case WP_FIREMODE_20MM:		//added 1/12/05 by pimpy
			if ( buttons & BUTTON_ATTACK )
			{
				buttons |= BUTTON_ALT_ATTACK;
				buttons &= ~BUTTON_ATTACK;
...
static void PM_Weapon( void )
{
	...
	{
#ifdef SMKWEAPONS
		if( (attackButtons&BUTTON_ALT_ATTACK && pm->ps->firemode[pm->ps->weapon] != WP_FIREMODE_20MM) || (attackButtons&BUTTON_ATTACK && pm->ps->firemode[pm->ps->weapon] == WP_FIREMODE_20MM) || (pm->ps->pm_flags & PMF_ZOOM_REZOOM) )
#else
