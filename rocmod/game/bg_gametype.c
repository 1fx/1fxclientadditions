...
Returns true if the specified gametypes are related.
===============
*/
qboolean BG_RelatedGametype ( int gametype, const char* name )
{
	// Do the names match?
	if ( !Q_stricmp ( bg_gametypeData[gametype].name, name )  )
	{
		return qtrue;
	}
	// Are they at least related?
	else if ( !Q_stricmp ( bg_gametypeData[gametype].basegametype, name )  )
	{
		return qtrue;
	}
