...
static void CG_Obituary( entityState_t *ent )
{
	...
	{
		case TEAM_RED:
			targetColor = cgs.teamData.teamcolor[1];
			break;

		case TEAM_BLUE:
			targetColor = cgs.teamData.teamcolor[2];
			break;
	}
	...
	if ( cgs.gametypeData->showKills )
	{
		if ( attacker == cg.snap->ps.clientNum )
		{
			...
			if ( !cgs.gametypeData->teams )
			{
				s = va("You killed %s%s\n%s place with %i\n%s", targetColor, targetName,
					CG_PlaceString( cg.snap->ps.persistant[PERS_RANK] + 1 ),
					cg.snap->ps.persistant[PERS_SCORE],
					cg.centerPrintPriority?cg.centerPrint:"" );
			}
			else
			{
				s = va("You killed %s%s\n%s", targetColor, targetName,
	...
	else
	{
		...
		{
			case TEAM_RED:
				attackerColor = cgs.teamData.teamcolor[1];
				break;

			case TEAM_BLUE:
				attackerColor = cgs.teamData.teamcolor[2];
				break;
		}
	}
	...
	{
		bodypartstr = "";
		if ( cg_drawDetailedObituary.integer == 1 )
		{
			switch ( shotlocation )
			{
				case HL_FOOT_RT:
		...
		else if ( cg_drawDetailedObituary.integer == 2 )
		{
			switch ( shotlocation )
			{
				case HL_FOOT_RT:
					bodypartstr = "in the right foot ";
					break;
				case HL_FOOT_LT:
					bodypartstr = "in the left foot ";
					break;
				case HL_LEG_UPPER_RT:
					bodypartstr = "in the right thigh ";
					break;
				case HL_LEG_UPPER_LT:
					bodypartstr = "in the left thigh ";
					break;
				case HL_LEG_LOWER_RT:
					bodypartstr = "in the right calf ";
					break;
				case HL_LEG_LOWER_LT:
					bodypartstr = "in the left calf ";
					break;
				case HL_HAND_RT:
					bodypartstr = "in the right hand ";
					break;
				case HL_HAND_LT:
					bodypartstr = "in the left hand ";
					break;
				case HL_ARM_RT:
					bodypartstr = "in the right arm ";
					break;
				case HL_ARM_LT:
					bodypartstr = "in the left arm ";
					break;
				case HL_HEAD:
					bodypartstr = "in the head ";
					break;
				case HL_WAIST:
					...
					break;
				case HL_BACK_RT:
					bodypartstr = "in the right back ";
					break;
				case HL_BACK_LT:
					bodypartstr = "in the left back ";
					break;
				case HL_BACK:
					bodypartstr = "in the back ";
					break;
				case HL_CHEST_RT:
					bodypartstr = "in the right chest ";
					break;
				case HL_CHEST_LT:
					bodypartstr = "in the left chest ";
					break;
				case HL_CHEST:
					bodypartstr = "in the chest ";
					break;
		...
		{
			case MOD_KNIFE:
				message = va("was sliced %sby", bodypartstr);
				break;
			...
			case MOD_M590_SHOTGUN:
				...
				{
					message = va("was bludgeoned %sby", bodypartstr);
					message2 = va("'s %s", weaponParseInfo[mod].mName );
				}
				else
				{
					message = va("was pumped full of lead %sby", bodypartstr);
					message2 = va("'s %s", weaponParseInfo[mod].mName );
				}
				break;
			...
			case MOD_SILVER_TALON:
				if ( attack == ATTACK_ALTERNATE )
				{
					message = va("was pistol whipped %sby", bodypartstr);
					message2 = va("'s %s", weaponParseInfo[mod].mName );
				}
				else
				{
					message = va("was shot %sby", bodypartstr);
					message2 = va("'s %s", weaponParseInfo[mod].mName );
				}
				break;
			...
			case MOD_AK74_ASSAULT_RIFLE:
				if ( attack == ATTACK_ALTERNATE )
				{
					message = va("was stabbed %sby", bodypartstr);
				}
				else
				{
					message = va("was shot %sby", bodypartstr);
			...
			case MOD_MSG90A1_SNIPER_RIFLE:
				message = va("was sniped %sby", bodypartstr);
				message2 = va("'s %s", weaponParseInfo[mod].mName );
				break;
		...
		if (message) {
			Com_Printf( "%s%s %s %s%s%s %s\n", targetColor, targetName, message, attackerColor, attackerName, message2, shotstr);
			return;
		}
	}
...
void CG_EntityEvent( centity_t *cent, vec3_t position )
{
	...
	case EV_BOTSUBWAYPOINT:
		DEBUGNAME("EV_BOTSUBWAYPOINT");
		CG_TestLine(cent->lerpOrigin, es->angles, 30000, 0xff00ff, 3);
		//Just render for 30 seconds because this waypoint might not be rendered again for quite some time.
		break;
