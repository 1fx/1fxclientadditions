...
/*
=================
CG_UpdateCustomTeams
=================
*/
void CG_UpdateCustomTeams ( void )
{
	if ( cg_allowCustomTeams.integer )
	{
		const char *string;
		int i;
		float f;

		string = cg_customClanShade.string;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.clanShade[0] = f;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.clanShade[1] = f;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.clanShade[2] = f;
		cgs.teamData.teamcolor[1][0] = '^';
		cgs.teamData.teamcolor[1][1] = cg_customRedCode.string[0];
		strncpy( cgs.teamData.redName, cg_customRedName.string, sizeof(cgs.teamData.redName) - 1 );
		cgs.teamData.redName[ sizeof(cgs.teamData.redName) - 1 ] = 0;
		string = cg_customRedShade.string;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.redShade[0] = f;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.redShade[1] = f;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.redShade[2] = f;
		cgs.teamData.teamcolor[2][0] = '^';
		cgs.teamData.teamcolor[2][1] = cg_customBlueCode.string[0];
		strncpy( cgs.teamData.blueName, cg_customBlueName.string, sizeof(cgs.teamData.blueName) - 1 );
		cgs.teamData.blueName[ sizeof(cgs.teamData.blueName) - 1 ] = 0;
		string = cg_customBlueShade.string;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.blueShade[0] = f;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.blueShade[1] = f;
		i = _atoi( &string );
		f = i;
		f /= 100;
		cgs.teamData.blueShade[2] = f;
	}
	else
	{
		VectorSet( cgs.teamData.clanShade, 1.0f, 0.5f, 0.0f );
		cgs.teamData.teamcolor[1][0] = '^';
		cgs.teamData.teamcolor[1][1] = '1';
		strncpy( cgs.teamData.redName, "Red", sizeof(cgs.teamData.redName) - 1 );
		cgs.teamData.redName[ sizeof(cgs.teamData.redName) - 1 ] = 0;
		VectorSet( cgs.teamData.redShade, 1.0f, 0.1f, 0.1f );
		cgs.teamData.teamcolor[2][0] = '^';
		cgs.teamData.teamcolor[2][1] = '4';
		strncpy( cgs.teamData.blueName, "Blue", sizeof(cgs.teamData.blueName) - 1 );
		cgs.teamData.blueName[ sizeof(cgs.teamData.blueName) - 1 ] = 0;
		VectorSet( cgs.teamData.blueShade, 0.3f, 0.3f, 1.0f );
	}
}

/*
=================
CG_UpdateExtraFeatures
=================
*/
void CG_UpdateExtraFeatures ( void )
{
	int			i;
	int			value;
	int			last;
	...
	last = cg.extraFeatures & EXTRA_SPECLIST;

	value |= (cg_drawTeamInfo.integer||cg_drawTeamOverlay.integer)?EXTRA_TEAMINFO:0;
	value |= cg_drawSpecList.integer?EXTRA_SPECLIST:0;

	if ( value == cg.extraFeatures )
	{
		if ( cg.extraFeaturesAckTime && cg.time > cg.extraFeaturesAckTime + 1000 )
		{
			cg.extraFeaturesAckTime = cg.time;
			trap_SendClientCommand( va("uef %i", value ) );
		}
		return;
	}

	cg.extraFeatures = value;
	cg.extraFeaturesAckTime = cg.time;

	trap_SendClientCommand( va("uef %i", value ) );

	if ( !cg_drawSpecList.integer && last )
	{
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	if ( cg.updateTimer < cg.time )
	{
		cg.updateTimer = cg.time + 1000;
		CG_UpdateCustomTeams();
	}
	...
	if ( ((inMatch.integer == 1 && cg_autoDemo.integer) || cgs.demoRequested) && cgs.demoRunning != 2 )
	{
		if ( !cgs.demoTime )
		{
			trap_Cvar_Set( "g_synchronousClients", "1" );
			cgs.demoTime = cg.time + 100;
		}
		else if ( cg.time > cgs.demoTime )
		{
			if ( !cgs.demoRunning )
			{
				if ( inMatch.integer == 1 )
				{
					qtime_t			currtime;

					trap_RealTime (&currtime);
					trap_SendConsoleCommand( va("record match-%i%02i%02i-%02i%02i", currtime.tm_year + 1900, currtime.tm_mon + 1, currtime.tm_mday, currtime.tm_hour, currtime.tm_min) );
				}
				else
				{
	...
	else if ( (inMatch.integer == 0 && !cgs.demoRequested) && cgs.demoRunning )
	{
		trap_SendConsoleCommand( "stoprecord" );
		cgs.demoRunning = qfalse;
		cgs.demoTime = 0;
	}
}
