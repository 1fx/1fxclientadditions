...
void CG_NewClientInfo( int clientNum ) 
{
	...
	// dummy
	v = Info_ValueForKey( configstring, "dummy" );
	newInfo.dummy = atoi( v );
...
static void CG_PlayerFloatSprite( centity_t *cent, qhandle_t shader ) 
{
	...
	if ( cg_drawFriend.integer == 2 && cent->currentState.clientNum <= MAX_CLIENTS )
	{
		float x;
		float y;

//		Com_Printf("%sUsing player name entry %i of %i\n", (cg.playerNameCount >= MAX_CLIENTS)?"^1":"", cg.playerNameCount, MAX_CLIENTS );
		ent.origin[2] += 15;
		CG_WorldCoordToScreenCoordFloat( ent.origin, &x, &y );
//		cg.playerNames[cg.playerNameCount].clientNum = cent->currentState.clientNum;
		cg.playerNames[cent->currentState.clientNum].used = 1;
		cg.playerNames[cent->currentState.clientNum].x = x;
		cg.playerNames[cent->currentState.clientNum].y = y;
		cg.playerNames[cent->currentState.clientNum].scale = Distance( cent->lerpOrigin, cg.snap->ps.origin );
//		cg.playerNameCount++;
	}
	else
	{
...
//	}
	...
	{
		if ( cgs.clientinfo[ cent->currentState.clientNum ].dummy )
		{
			CG_PlayerFloatSprite( cent, cgs.media.dummyShader );
		}
		else if (cg_drawFriend.integer && cg.snap->ps.persistant[PERS_TEAM] == team && cgs.gametypeData->teams) 
		{
