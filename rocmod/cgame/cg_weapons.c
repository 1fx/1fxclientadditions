...
void CG_PredictedBullet ( centity_t* cent, attackType_t attack )
{
	...
	if ( detailed )
	{
		...
		{
			if ( ent->weapon == WP_MSG90A1 && cg_realisticSniper.integer )
			{
				inaccuracy *= JUMP_SNIPER_ACCURACY_MODIFIER;
			}
			else
			{
		...
		else if ( VectorLength(cg.predictedPlayerState.velocity) > (cg.predictedPlayerState.speed * 0.75f) )
		{
			if ( ent->weapon == WP_MSG90A1 && cg_realisticSniper.integer )
			{
				inaccuracy *= RUN_SNIPER_ACCURACY_MODIFIER;
			}
		}
