...
void CG_Respawn( void )
{
	...
	if(cg.thirdViewMsgShown != cg.time && cgs.allowThirdPerson && cgs.clientinfo[cg.predictedPlayerState.clientNum].team != TEAM_SPECTATOR && cg_thirdPerson.integer == 0 && (cgs.current_gametype == GT_HS || cgs.current_gametype == GT_HZ)){
		Com_Printf("^3[Info] ^7You're playing %s.\n", (cgs.current_gametype == GT_HS) ? "Hide&Seek" : "Zombies");
